...
parser = argparse.ArgumentParser(description='Model training program for flower dataset using Pytorch', usage='''
        ...
        python train.py data_dir --learning_rate 0.01 --hidden_units [1024, 512, 256] --epochs 20 (set hyperparameters)''', prog='train')
...
## Choose architecture:
parser.add_argument('--arch', action="store", default="vgg16", choices=['vgg13', 'vgg16', 'vgg19'], help="you can only choose vgg13, vgg16 or vgg19", dest="architecture")

## Set hyperparameters
parser.add_argument('--learning_rate', action="store", default="0.001", type=float, help="Set Learning rate", dest="learning_rate")
parser.add_argument('--hidden_units', action="store", nargs=3, default=[1024, 512, 256], type=int, help="enter 3 integers between 25088 and 102 in decreasing order", dest="hidden_units")
